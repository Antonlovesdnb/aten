<#
.SYNOPSIS
  Compile fishbowl.man into the resource DLL (fishbowl_events.dll) that the
...
.DESCRIPTION
  Runs the message compiler -> resource compiler -> linker chain:
      mc.exe -um fishbowl.man            (=> fishbowl.h, fishbowl.rc, *.bin)
      rc.exe /fo fishbowl.res fishbowl.rc
      link.exe /DLL /NOENTRY ...         (=> fishbowl_events.dll)
  ...
  whenever fishbowl.man changes. The produced DLL is what 'fishbowl install'
  copies to %ProgramData%\fishbowl and registers via 'wevtutil im'.

  After building, register (admin):
      wevtutil im fishbowl.man /rf:<full path to fishbowl_events.dll> /mf:<same>
  Inspect:  wevtutil gl Fishbowl/Operational
  Remove:   wevtutil um fishbowl.man
...
# --- compile ---
Write-Host "==> mc -um fishbowl.man"
& $mc -um fishbowl.man

Write-Host "==> rc fishbowl.rc"
& $rc /nologo /fo fishbowl.res fishbowl.rc

Write-Host "==> link fishbowl_events.dll"
& $link /DLL /NOENTRY /NOLOGO "/MACHINE:$($Arch.ToUpper())" /OUT:fishbowl_events.dll fishbowl.res

if (-not (Test-Path fishbowl_events.dll)) { throw "link did not produce fishbowl_events.dll" }
Write-Host ""
$dll = Resolve-Path fishbowl_events.dll
Write-Host "Built: $dll"
Write-Host "Register (admin):  wevtutil im fishbowl.man /rf:`"$dll`" /mf:`"$dll`""
