...
pub struct OutputSection {
    ...
    /// `%ProgramData%\aten\events.jsonl`. CLI test runs typically
    /// override to a temp path.
    pub file_path: Option<PathBuf>,
    /// Which sink(s) to write to: `jsonl` | `eventlog` | `both`. Defaults to
    /// `jsonl`. `eventlog`/`both` are Windows-only (the ATEN/Operational
...
pub fn default_config_path() -> PathBuf {
    ...
    #[cfg(target_os = "linux")]
    {
        PathBuf::from("/etc/aten/config.toml")
    }
    #[cfg(not(any(target_os = "windows", target_os = "linux")))]
    {
        PathBuf::from("./aten.toml")
    }
}
