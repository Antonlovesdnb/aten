...
//!
//! Subcommands:
//! - `aten version` — prints schema and crate version
//! - `aten transcript <path>` — parses a Claude Code transcript JSONL,
...
pub(crate) fn parse_sink_arg(s: Option<String>) -> sink::SinkKind {
    s.map(|s| {
        sink::SinkKind::parse(&s).unwrap_or_else(|| {
            eprintln!("aten: unrecognised output sink '{s}'; using jsonl");
...
#[derive(Parser)]
#[command(name = "aten", version, about = "ATEN daemon CLI")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
enum Command {
    ...
    /// Install aten as a system service so it survives reboots. On Windows:
    /// the `atensvc` SCM service (LocalSystem, auto-start) + the
    /// `ATEN/Operational` Event Log channel. On Linux: writes
    /// `/etc/systemd/system/aten.service` and `systemctl enable --now`s it.
    /// Drops a default config if none exists. Needs admin/root. Re-running
    /// upgrades an existing install in place.
    #[cfg(any(target_os = "windows", target_os = "linux"))]
    Install,
    /// Stop and remove the aten service (SCM service on Windows, systemd
    /// unit on Linux). Leaves config + event log on disk. Needs admin/root.
    #[cfg(any(target_os = "windows", target_os = "linux"))]
    Uninstall,
    /// Service entry point. Invoked by SCM (not by humans typing).
    /// `aten install` registers this subcommand as the service's
    /// launch argument; SCM then calls `aten service` when it starts
    ...
    CollectWindows {
        ...
        /// write to the Windows Event Log channel `ATEN/Operational`
        /// (needs the manifest registered via `aten install`).
        #[arg(long)]
        sink: Option<String>,
    },
    ...
    Daemon {
        ...
        /// (`%ProgramData%\aten\config.toml` on Windows). Missing
        ...
        /// `ATEN/Operational` (Windows only; needs `aten install`).
        #[arg(long)]
        sink: Option<String>,
    },
}
...
/// CLI (`aten daemon ...`) and the Windows service entry point
...
pub(crate) fn run_daemon_loop_windows(
    ...
    aten_collector_windows::run_with_tick(
        ...
        || {
            ...
            let now = Instant::now();
            let drained: Vec<aten_schema::Event> = {
    ...
    let remaining: Vec<aten_schema::Event> = {
...
fn run_collect_windows(
    ...
    eprintln!(
        "aten windows collector starting (agents = {:?}, sink = {:?})",
        cfg.enrolled_agents, sink_kind
    );

    aten_collector_windows::run(cfg, stop, move |event| {
        let mut s = sink_for_emit.lock().expect("sink lock");
        s.emit(&event);
        s.flush();
    })?;

    eprintln!("aten windows collector stopped");
    Ok(())
}
...
fn windows_cwd_for_pid(pid: i32) -> Option<String> {
    ...
    aten_collector_windows::query_cwd(pid as u32)
}
...
fn run_daemon(
    ...
    let engine = AttributionEngine::new(EngineConfig {
        cwd_for_pid: aten_attribution::default_cwd_for_pid,
        transcript_paths: inputs.transcript_paths.clone(),
        host_id: read_machine_id(),
        user_for_transcript: linux_user_for_transcript,
        state_path: Some(std::path::PathBuf::from("/var/lib/aten/state.json")),
    });
    ...
    aten_collector_linux::run_with_tick(
        cfg,
        stop,
        |event| {
        ...
        || {
            ...
            let now = Instant::now();
            let mut drained: Vec<aten_schema::Event> = Vec::new();
            {
...
// (`aten_collector_macos`), the host_id source, the cwd/user resolvers, and
...
fn macos_user_for_transcript(path: &std::path::Path) -> Option<String> {
    aten_collector_macos::file_owner(path)
}
...
fn macos_cwd_for_pid(pid: i32) -> Option<String> {
    aten_collector_macos::query_cwd(pid as u32)
}

/// One-shot macOS collector (`aten collect-macos`): emits raw events to a
...
fn run_daemon(
    ...
    let engine = AttributionEngine::new(EngineConfig {
        ...
        state_path: Some(std::path::PathBuf::from(
            "/Library/Application Support/aten/state.json",
        )),
    });
    let engine = RefCell::new(engine);
    let _ = engine.borrow_mut().refresh()?;
