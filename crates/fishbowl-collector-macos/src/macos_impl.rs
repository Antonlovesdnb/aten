...
use fishbowl_schema::{Attribution, Event, Process};
    ...
    T: FnMut(),
{
    ...
    // Seed already-running agents — ESF NOTIFY_EXEC only fires post-subscribe.
    {
        ...
        eprintln!("fishbowl-macos: enrollment rundown seeded {seeded} agent root(s)");
    }
    ...
    let _es_guard = {
        match crate::esf::start(config.clone(), state.clone(), tx.clone()) {
            ...
            Err(e) => {
                ...
                eprintln!("fishbowl-macos: ESF disabled ({e:#}); running netflow-only");
                None
            }
        }
    };
    ...
    eprintln!(
        "fishbowl-macos: collector started (agents = {:?})",
...
pub fn process_from_snapshot(
    ...
    parent_chain: Vec<fishbowl_schema::ParentChainEntry>,
) -> Process {
    Process {
        pid: snap.pid,
        ppid: snap.ppid,
