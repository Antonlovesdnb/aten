...
//! - `install_service()` (`fishbowl install`): writes a default config if none
//!   exists, drops `/etc/systemd/system/fishbowl.service`, and
//!   `systemctl enable`s + restarts it. Needs root.
//! - `uninstall_service()` (`fishbowl uninstall`): `systemctl disable --now`s
...
//! systemd invokes `fishbowl daemon` directly per the unit's `ExecStart`.
//!
...
const UNIT_NAME: &str = "fishbowl.service";
const UNIT_PATH: &str = "/etc/systemd/system/fishbowl.service";
const CONFIG_DIR: &str = "/etc/fishbowl";
const CONFIG_PATH: &str = "/etc/fishbowl/config.toml";
const LOG_DIR: &str = "/var/log/fishbowl";
const STATE_DIR: &str = "/var/lib/fishbowl";
const EVENTS_PATH: &str = "/var/log/fishbowl/events.jsonl";

pub fn install_service() -> Result<()> {
    if !is_root() {
        bail!("`fishbowl install` needs root (sudo) — it writes a systemd unit and the daemon needs CAP_BPF/CAP_SYS_ADMIN/CAP_PERFMON");
    }
    ...
    if cfg_path.exists() {
        eprintln!("[fishbowl install] preserved existing config {CONFIG_PATH}");
    } else {
        let home = real_user_home();
        let body = format!(
            "# fishbowl-v2 service config. Edit and restart the service:\n\
             #   sudo systemctl restart fishbowl\n\
             \n\
             [daemon]\n\
        ...
        eprintln!("[fishbowl install] wrote default config to {CONFIG_PATH}");
    }
    ...
    eprintln!("[fishbowl install] wrote systemd unit {UNIT_PATH}");
    ...
    if systemctl(&["restart", UNIT_NAME]) {
        eprintln!("[fishbowl install] service 'fishbowl' enabled and started");
    } else {
        eprintln!(
            "[fishbowl install] unit installed but start failed — inspect with: \
             journalctl -u fishbowl -e"
        );
    }
    Ok(())
}
...
pub fn uninstall_service() -> Result<()> {
    if !is_root() {
        bail!("`fishbowl uninstall` needs root (sudo)");
    }
    // disable --now stops the service and removes the WantedBy symlink.
    systemctl(&["disable", "--now", UNIT_NAME]);
    match std::fs::remove_file(UNIT_PATH) {
        Ok(()) => eprintln!("[fishbowl uninstall] removed {UNIT_PATH}"),
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => {}
        Err(e) => eprintln!("[fishbowl uninstall] could not remove {UNIT_PATH}: {e} (non-fatal)"),
    }
    systemctl(&["daemon-reload"]);
    eprintln!(
        "[fishbowl uninstall] service 'fishbowl' removed; left {CONFIG_DIR}, {STATE_DIR}, and \
         {LOG_DIR} in place"
    );
    Ok(())
}
