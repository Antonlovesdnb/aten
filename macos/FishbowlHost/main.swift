// FishbowlHost — minimal host application whose only jobs are to (1) activate
...
//
// Usage:
//   open macos/build/FishbowlHost.app          # activate + enable filter
//   FishbowlHost --deactivate                   # tear down (best effort)
...
private let log = Logger(subsystem: "ai.fishbowl.host", category: "host")

// Keep this in sync with the system extension's bundle identifier in project.yml.
private let extensionBundleID = "ai.fishbowl.netext"
    ...
    private func enableContentFilter() {
        ...
        mgr.loadFromPreferences { [weak self] loadErr in
            ...
            if mgr.providerConfiguration == nil {
                ...
                mgr.localizedDescription = "Fishbowl"
            }
            mgr.isEnabled = true
